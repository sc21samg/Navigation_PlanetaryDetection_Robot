...
class RoboNaut(Node):
    def __init__(self):
        ...
        self.windows_ss_dir = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13'
        ...
        #empty folders
        save_directory = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13'
        clear_directory(save_directory)           
            
        folder_path = '/uolstore/home/users/sc21samg/ros2_ws/src'
        delete_planet_images(folder_path)

        folder_path = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/image_detection/extra_images'
    ...
    def camera_callback(self, data):
            ...
            for contour in contours:
                ...
                if len(approx) == 4:
                    ...
                    if has_white_border and black_ratio > 0.5:
                        ...
                        filename = f'/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13/window{self.window_screenshot_counter}.png'
        ...
        if self.in_green_room and self.robot_stoped and not self.full_rotation:
            ...
            self.get_logger().info("Start stitching the windows.")
            self.flag_window_processing0 = 1
            self.detect_image_rename()
            rclpy.shutdown() 
         
        #stages of image detection
        #comented out for now, should be == 1
    ...
    def attempt_capture(self):
        ...
        if self.robot_close:
            ...
            if self.contour >= 0:
                filename = f'/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13/window{self.contour}.png'
    ...
    def prepare_capture(self):
        ...
        if self.contour == 0 and not self.prepare_capture_called:
            ...
            self.get_logger().info("Group13 folder cleared for new captures.")
    ...
    def detect_image_rename(self):
        #detects the planet name in each function and saves them
        if self.flag_window_processing0 == 1:
            
            self.get_logger().info(f"Detecting planets and renameing images")
    
            model_path = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/data_training/model.pth'
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            model = load_model(model_path, num_classes=4)
            model.to(device)
            #add the actual ss windows folder location here
            image_directory = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13'
            #image_detection/test_windows
            images = [os.path.join(image_directory, f) for f in os.listdir(image_directory) if f.endswith('.png')]

            for image_path in images:
                img = cv2.imread(image_path, cv2.IMREAD_COLOR)
                if img is None:
                    self.get_logger().info(f"Failed to load image from {image_path}.")
                    continue
                ...
                if not planet_details:
                    self.get_logger().info("No planets detected in the image.")
                    continue
                ...
                new_image_directory = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13'
                save_path = os.path.join(new_image_directory, unique_filename)
                self.get_logger().info(f"Saved processed image with detected planets at: {save_path}")
    ...
    def good_stitching(self):
        #stitches the images together in a panorama
        if self.flag_window_processing1 == 1:
            
            self.get_logger().info(f"Creating the panorama")
            
            # Directory containing the images
            folder_path = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13'
            ...
            for name, img in images.items():
                if img is not None:
                    if base_image is None:
                        base_image = img
                        self.get_logger().info(f"Initialized panorama with {name}.")
                    else:
                        base_image = stitch_images(base_image, img)
                        self.get_logger().info(f"Added {name} to panorama.")

            if base_image is not None:
                output_path = os.path.join(folder_path, 'panorama.png')
                cv2.imwrite(output_path, base_image)
                self.get_logger().info(f"Final panorama saved as '{output_path}'.")
            else:
                self.get_logger().info("No images were found for stitching.")

            # Load the image
            image_path = '/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13/panorama.png'
            #image_path = '/uolstore/home/users/sc21samg/ros2_ws/src/final_panorama (1).jpg'
    ...
    def planet_measurements(self):
        #measures the distance between planets and outputs in the txt file
        if self.flag_window_processing2 == 1:
            
            self.get_logger().info(f"Planet measurements")
            ...
            if img is None:
                self.get_logger().info("Failed to load image.")
                return
            save_directory = '/uolstore/home/users/sc21samg/ros2_ws/src'
            image_height_pixels, image_width_pixels, _ = img.shape
            ...
            with open("/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13/measurements.txt", "w") as file:
                file.write("")
            ...
            if planet_details:
                ...
                for detail in planet_details:
                    filename = f"planet_{len(planet_positions)+1}.png"
                    cropped_image = locate_and_crop(filename)
                    cropped_image_path = "/uolstore/home/users/sc21samg/ros2_ws/src/group_project/image_detection/extra_images/cropped_planet.jpg"
                    cropped_image.save(cropped_image_path)  # Save the cropped image for prediction
                ...
                for index, pos in enumerate(planet_positions):
                    ...
                    with open("/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13/measurements.txt", "a") as file:
                        self.count_images = self.count_images + 1
                        file.write(f"{pos[3]}: {int(pos[2])} km\n")
                        self.get_logger().info(f"{pos[3]}: {int(pos[2])} km") 
                        self.get_logger().info(f"Detected {pos[3]} at (x={pos[0]}, y={pos[1]}) with height {pos[4]} pixels and distance {int(pos[2])} km. \n")
                    ...
                    for j in range(i + 1, len(planet_positions)):
                        ...
                        with open("/uolstore/home/users/sc21samg/ros2_ws/src/group_project/group13/measurements.txt", "a") as file:
                            if len(planet_positions) == 2: 
                                ...
                                self.get_logger().info(f"Distance: {int(real_distance_km)} km")
                            else:
                                file.write(f"Distance between {planet_positions[i][3]} and {planet_positions[j][3]}: {int(real_distance_km)} km\n")
                                self.get_logger().info(f"Distance between {planet_positions[i][3]} and {planet_positions[j][3]}: {int(real_distance_km)} km\n")
        
                
                cv2.imwrite('/uolstore/home/users/sc21samg/ros2_ws/src/group_project/image_detection/extra_images/detected_planets.png', img)
                #self.flag_window_processing2 = 0
                folder_path = '/uolstore/home/users/sc21samg/ros2_ws/src'
                #delete_planet_images(folder_path)
            ...
            if not is_folder_empty(folder_path):
                ...
                self.get_logger().info(f"Couldn't detect any planets for measurement")
    ...
    def rotate_robot(self):
        ...
        rotation_velocity = Twist()
        rotation_velocity.angular.z = math.pi / 9  # Rotate with 30 degrees per second
        ...
        if sign_color == 'red':
            ...
            if self.sign_detected_post_rotation:
                ...
                if self.current_entrance == 1:
                    self.get_logger().info('Green sign detected at entrance 1. Proceeding to center 1.')
                    self.proceed_to_center(1)
                    self.in_green_room = True
                elif self.current_entrance == 2:
                    self.get_logger().info('Green sign detected at entrance 2. Proceeding to center 2.')
                    self.proceed_to_center(2)
                    self.in_green_room = True
